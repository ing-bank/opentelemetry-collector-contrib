...
package inmemorymetricexporter // import "github.com/open-telemetry/opentelemetry-collector-contrib/exporter/prometheusexporter"
...
func (a *lastValueAccumulator) accumulateSummary(metric pmetric.Metric, il pcommon.InstrumentationScope, resourceAttrs pcommon.Map, now time.Time) (n int) {
	...
	for i := 0; i < dps.Len(); i++ {
		...
		if ip.Flags().NoRecordedValue() {
			a.registeredMetrics.Delete(metric.Name())
			return 0
		}

		v, ok := a.registeredMetrics.Load(metric.Name())
		stalePoint := ok &&
		...
		a.registeredMetrics.Store(metric.Name(), &accumulatedValue{value: m, resourceAttrs: resourceAttrs, scope: il, updated: now})
		n++
	}
